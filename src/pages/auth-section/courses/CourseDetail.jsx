...
<div className="w-full md:w-2/3">
      ...
      <div className="text-2xl md:text-4xl font-bold mb-4">{course.title}</div>
      ...
      {showMoreDetails && (
        <div className="mt-4">
          <div className="text-2xl font-bold mb-4">Who Should Take This Course</div>
          <div><strong></strong> {course.whoShouldTake}</div>
          <div className="text-2xl font-bold mt-4 mb-4">Pre-requisites:</div>
          <div><strong></strong> {course.prerequisites}</div>
          <div className="text-2xl font-bold mt-4 mb-4">Tools and Technologies Covered</div>
          <div><strong></strong> {course.toolsAndTech}</div>
          <div className="text-2xl font-bold mt-4 mb-4">Course Features</div>
          <div> {course.courseFeatures}</div>
          <div className="text-2xl font-bold mt-4 mb-4">Learning Outcomes</div>
          <div> {course.learningOutcomes}</div>
          <div className="text-2xl font-bold mt-4 mb-4">Access</div>
          <div><strong></strong> {course.access}</div>
        </div>
      )}
        <div className="text-sm text-gray-500 mb-8">Author: {course.author} | Price: {course.price}</div>

        <div className="bg-white shadow-lg rounded-lg p-6 mb-8">
          <div className="flex justify-between items-center">
            <div>
              <div className="text-lg font-semibold">Current Status</div>
              <div className="mt-2 bg-[#000] text-white px-6 py-2 rounded-[20px]">{course.enrolled ? 'Enrolled' : 'Not Enrolled'}</div>
            </div>
            ...
            <div>
              ...
              <button className="mt-2 bg-[#550CC8] text-white px-4 md:px-6 py-1 md:py-2 rounded-[20px]">Take this Course</button>
            </div>
          </div>
        </div>
        ...
        <div>
         {course.curriculum.map((module, index) => (
          <div className="bg-white shadow-lg rounded-lg p-6 mb-4" key={index}>
            <div className="flex justify-between items-center cursor-pointer" onClick={() => toggleModule(index)}>
              <div className="flex items-center">
                <FaPlay className="text-[#000] mr-4" />
                <div>
                  <div className="text-lg font-semibold">{module.moduleName}</div>
...
<img src={course.coverImage} alt="Course Cover" className="w-full object-cover mb-6 rounded"/>
          <button className="bg-gray-300  text-black px-6 py-2 rounded-[20px] mb-4 w-full">Not Enrolled</button>
          <button className="bg-[#550CC8] text-white px-6 py-2 rounded-[20px] mb-4 w-full"><NavLink to="/coursePlayer">Enroll Now</NavLink></button>
          <div className="text-sm text-gray-900 mb-2">This course includes:</div>
          <div className="text-sm text-gray-500 mb-2">Students: {course.numberOfStudents}</div>
          <div className="text-sm text-gray-500 mb-2">Lessons: {course.totalLessons}</div>
          <div className="text-sm text-gray-500">Exercises: {course.numberOfExercises}</div>
        </div>
      </div>
    </div>
  );
